...
void readNamedPipeNameForHECoupling(string* Name_Pipe_Write, string* Name_Pipe_Read)
{
	...
	*Name_Pipe_Write = NPW;
	cout << Name_Pipe_Read << endl;
	cout << Name_Pipe_Write << endl;
	cout << "...done." << endl;
}

void ReadStaticVariables(int* NumberPipes, double* ColThickness, double* BackfillPerm, double* DisconnectionPressure,
	...
	double* a22, double* a13, double* a31, int* ExchangeTimeStep, int* outputSteps,
	...
	vector<double>* PipeLengthsVector, vector<double>* DefectAreaVector, vector<double>* MeanWaterPressureVector, vector<double>* PipeDatumn)
{
	...
	*NumberPipes = atoi(DataArray.at(1).c_str());
	*ColThickness = atof(DataArray.at(3).c_str());
	*BackfillPerm = atof(DataArray.at(5).c_str());
	*DisconnectionPressure = atof(DataArray.at(7).c_str());
	*FluidDensity = atof(DataArray.at(9).c_str());
	*FluidViscosity = atof(DataArray.at(10).c_str());
	*a00 = atof(DataArray.at(12).c_str());
	*a10 = atof(DataArray.at(13).c_str());
	*a01 = atof(DataArray.at(14).c_str());
	*a20 = atof(DataArray.at(15).c_str());
	*a11 = atof(DataArray.at(16).c_str());
	*a02 = atof(DataArray.at(17).c_str());
	*a30 = atof(DataArray.at(18).c_str());
	*a21 = atof(DataArray.at(19).c_str());
	*a12 = atof(DataArray.at(20).c_str());
	*a03 = atof(DataArray.at(21).c_str());
	*a40 = atof(DataArray.at(22).c_str());
	*a31 = atof(DataArray.at(23).c_str());
	*a22 = atof(DataArray.at(24).c_str());
	*a13 = atof(DataArray.at(25).c_str());
	*ExchangeTimeStep = atoi(DataArray.at(27).c_str());
	*outputSteps = atoi(DataArray.at(28).c_str());
	*outputFormat = (DataArray.at(29).c_str());
	*LastTimeStep = atof(DataArray.at(30).c_str());
	...
	{
		PNV.push_back(atoi(DataArray.at(32 + (i * 7)).c_str()));
		UMV.push_back(atof(DataArray.at(33 + (i * 7)).c_str()));
		DMV.push_back(atof(DataArray.at(34 + (i * 7)).c_str()));
		PLV.push_back(atof(DataArray.at(35 + (i * 7)).c_str()));
		DAV.push_back(atof(DataArray.at(36 + (i * 7)).c_str()));
		MWPV.push_back(atof(DataArray.at(37 + (i * 7)).c_str()));
		PD.push_back(atof(DataArray.at(38 + (i * 7)).c_str()));
	}
	*PipeNameVector = PNV;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	ReadStaticVariables(&CP.NumberPipes, &CP.ColThickness, &CP.BackfillPerm, &CP.DisconnectionPressure,
		&CP.FluidDensity, &CP.FluidViscosity, &CP.a00, &CP.a10, &CP.a01, &CP.a20,
		&CP.a11, &CP.a02, &CP.a30, &CP.a21, &CP.a12, &CP.a03, &CP.a40,
		&CP.a22, &CP.a13, &CP.a31, &CP.ExchangeTimeStep, &CP.outputSteps,
	...
	while (Extran_ok == true)
	{
		for (long t = 0; t <= CP.LastTimeStep; t += CP.ExchangeTimeStep)
		{
			...
			{
				CP.PipeWaterLevel.push_back(((CP.PipeWaterLevelDatumn[CP.UpstreamManholeVector[l]] + CP.PipeWaterLevelDatumn[CP.DownstreamManholeVector[l]]) / 2) - CP.PipeDatumn[l]);
			}
			printf("-");
			for (int i = 0; i < anzahlHaltungen; i++)
			{
				if (find(CP.HEPipeNames.begin(), CP.HEPipeNames.end(), to_string(CP.PipeNameVector[i])) != CP.HEPipeNames.end())
				{
					...
					//
					if (CP.MeanWaterPressureVector[i] / (CP.FluidDensity * 9.81) < CP.PipeWaterLevel_C[i])
					{
						if (CP.MeanWaterPressureVector[i] < CP.DisconnectionPressure)
						{
					...
					// HIER EIN PROBLEM. WASSERSTÄNDE WERDEN
			...
			// Code verschluckt sich irgendwie - Vielleicht hift es das hier unten einzufügen
